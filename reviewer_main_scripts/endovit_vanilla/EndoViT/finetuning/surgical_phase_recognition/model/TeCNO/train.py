"""
Reviewer note:
This is the shared Lightning entrypoint for the vanilla EndoViT fine-tuning
pipeline. In the reviewer bundle it mainly documents the training side of the
laparoscopic-to-robotic injection experiments used in E3.
"""
...
def train(hparams, ModuleClass, ModelClass, DatasetClass, logger):
    ...
    # ------------------------
    # The config file decides which backbone and temporal head to instantiate.
    ...
    trainer = Trainer(
        ...
        callbacks=[checkpoint_callback, early_stop_callback, ModelSummary(max_depth=-1)],
    ...
    trainer.fit(module, ckpt_path=hparams.resume_from_checkpoint, weights_only=False)
    # Print the selected checkpoint explicitly so it can be matched to the
    # archived run folders released with the paper.
