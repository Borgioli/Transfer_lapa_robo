"""
Reviewer note:
This is the shared Lightning entrypoint for phase-recognition fine-tuning in
the modified `endovit` pipeline. The concrete module, model, and dataset are
selected from the YAML config, which is why a single script covers the
different E1 and E2 training regimes.
"""
...
def train(hparams, ModuleClass, ModelClass, DatasetClass, logger):
    """
    Build the configured model stack, fit it, and then re-evaluate using the
    best checkpoint selected by validation performance.
    """
    # ------------------------
    # 1 INIT LIGHTNING MODEL
    # ------------------------
    # Instantiate the three components separately because the codebase swaps
    # backbones, temporal heads, and datasets through the config file.
    ...
    # Handle gpus argument for Lightning 2.x compatibility
    if hparams.gpus == -1:
        accelerator = "auto"
        devices = "auto"
    else:
    ...
    trainer.fit(module)
    # Print the selected checkpoint explicitly so the reviewer can match the
    # saved file against the archive structure described in the README.
    ckpt = torch.load(checkpoint_callback.best_model_path, weights_only=False)
    epoch = ckpt["epoch"]
    print(
    ...
    trainer.test(ckpt_path=checkpoint_callback.best_model_path, weights_only=False)
