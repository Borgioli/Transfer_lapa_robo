...
"""
Reviewer note:
This is the MAE pretraining entrypoint used for the robotic-only pretraining
pipeline summarized in the paper's E4 comparison. The implementation is kept
close to the original experiment code so the accompanying config snapshots
...
# Restrict ImageFolder datasets to named surgical subdatasets without changing
# the original class indexing expected by downstream code.
...
def get_args_parser():
    # The parser intentionally keeps the original argument names so the saved
    # YAML configs used during the experiments can be read without translation.
